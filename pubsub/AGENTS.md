...
- **Consumer groups route below containment.** `fanoutLocked` delivers each event to every accepting *ungrouped* subscription and to exactly one member of each *group* (`deliverToGroupLocked`) — but only among members that `accepts` the event, so taint clearance is enforced *before* group selection (an uncleared member is never a candidate). Selection is capacity-aware (`tryPlaceOnGroupLocked` prefers a member with buffer + in-flight room). Groups are live-only (no `--since`/retained, which would duplicate across competing consumers); per-group state (`groupRR`/`groupN`/`groupPend`) is reclaimed when the last member leaves. Keep group state bounded to live groups.
- **At-least-once (ack) groups.** With `SubOptions.Ack`, delivered events are held in `Subscription.inflight` until `Broker.Ack(sub, seq)`; on member close, un-acked events are requeued to the front of the group's bounded backlog (`groupPend`, capped by `MaxGroupPending`) and drained to a survivor — redelivery loses no work while the group keeps a member. A member at its in-flight cap (`SubQueue`) is skipped by selection. Never turn this into silent loss: overflow and last-member-leaves (non-durable) both increment `Dropped`.
- **Durable groups (cross-restart).** With `Options.GroupStore` set, the broker persists each group's **committed offset** = the highest seq below which nothing is outstanding (`recomputeGroupCommittedLocked`, `groupCmt`/`groupMax`); a durable group's last-member-leave keeps the offset (no drop) and the first member of a resuming group replays ring events after it (`resumeDurableGroupLocked`). A brand-new group starts at the current seq (no history replay). The core stays IO-agnostic: the file store lives in the root package behind `GroupStore` (like `EventLog`). Duplicates on replay are allowed (at-least-once); ring-aged-out gaps set `Truncated`.
...
- **Request/reply has no special path.** `Event.ReplyTo`/`Corr` (set via `PublishOptions`) are opaque correlation fields — validated (ReplyTo as a topic, Corr length-bounded), stamped, retained, and hashed like any other field. RPC is just a publish plus a correlated subscribe, so it inherits per-topic authorization and containment; do not add a broker-side reply router that would bypass them.
...
- `CGO_ENABLED=1 go test ./pubsub/ -race`. The suite covers deny-by-default, ordering, hash-chain tamper detection, both backpressure policies, rate limiting (fake clock), resource caps, label/taint containment, replay + truncation, fan-out isolation, consumer-group load balancing (+ capacity-awareness/containment/leave/replay-rejection), at-least-once ack groups (redelivery-on-loss, ack-releases-inflight, backlog-drains-on-ack), durable groups (cross-restart replay, brand-new-starts-from-now, total-outage resume), retained lifecycle (TTL/tombstone/federation/restart), and a concurrent fuzz.
