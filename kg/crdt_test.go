...
// TestMergeConverges proves the S8 CRDT property: two replicas that diverged
// offline reconcile to the same active set regardless of merge order, and a
// tombstone on either side removes the triple.
func TestMergeConverges(t *testing.T) {
	...
	if again := Merge(ab, a, b); !reflect.DeepEqual(again, ab) {
