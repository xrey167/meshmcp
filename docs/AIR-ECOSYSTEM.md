...
| **5 · Transactional Handoff v2** | **First slice shipped:** prepare → ready → commit/abort LIVE session-ownership move (`session.Server.MoveSessionTo` / `ServeMoveControl`), single-use destination grants (`air move grant`), generation fencing, and the full crash-recovery matrix (`-count=20`) + `storetest.RunSessionLiveMove` conformance. Remaining: operator-driven client redirection, checkpoint adapters beyond the two supported classes, and a Home action sheet. | Context Capsule v1, durable Activity identity, and runtime checkpoint support. |
