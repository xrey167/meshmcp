...
| Request-specific human approval | Beta | mesh HTTP | Mandatory approver ACL; approver identity from transport; request-bound, signed, SINGLE-USE approvals are shipped and enforced when a backend sets `approval_signing_key` (filter → `DecideToolCallBound` → atomic consume; see SECURITY-CLOSURE.md F-P3.2) | Without `approval_signing_key` the weaker per-(peer,tool) ambient grant applies; HTTP/remote backends now record `args_hash`/`policy_hash` on held calls and consume request-bound approvals identically | approver store dir | files | approvals + request-bound tests | Recommended with `approval_signing_key` + a tight TTL |
