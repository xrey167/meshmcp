...
| Live session move (Transactional Handoff v2, first slice) | Beta | stdio, HTTP | Deliberate operator/creator-initiated prepare→ready→commit relocation of ONE live session's ownership between two gateways (`session.Server.MoveSessionTo` / `ServeMoveControl`): the source serves at generation G until a SINGLE `TakeoverLease` generation-CAS (G→G+1), the destination only pre-warms (no lease/client) until it wins that CAS, and a crash at any step leaves exactly one resumable owner (source@G before, destination@G+1 after — never both). Destination is authorized by a CONSUMED single-use grant (`air move grant`), never an arbitrary peer | `MigrateHandshake` + `MigrateBackend` only (`MigrateFull`, no-checkpoint stateful, and degraded gen-0 refused at prepare); does NOT redirect the client (operator redirects mesh discovery); no exactly-once tool execution; the transfer trigger is gateway-to-gateway (the CLI ships only `air move grant`); cross-host needs PostgreSQL | optional PostgreSQL (`pgstore`); grant store (file) | files or PostgreSQL (`session_store`); grant store JSON | `session/move_test.go` crash matrix + abort + happy path (both modes, `-count=20`), `storetest.RunSessionLiveMove` (`MemStore` + live PostgreSQL), `air/move_grant_test.go` | Beta; cross-host needs the PostgreSQL store; pre-authorize with `air move grant`, redirect discovery to the destination as part of the drain |
