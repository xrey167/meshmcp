...
func TestRunAgentLoopDrivesRoleCalls(t *testing.T) {
	...
	err := runAgentLoop(ctx, mc, roleScripts["reader"], 3, 0, func(string, ...any) {})
	if err != nil {
		t.Fatal(err)
	}
...
func TestRunAgentLoopStopsAtCount(t *testing.T) {
	...
	if err := runAgentLoop(context.Background(), counter, roleScripts["billing"], 5, 0, func(string, ...any) {}); err != nil {
		t.Fatal(err)
	}
	if calls != 5 {
