...
| 🔁 **Sessions that survive** | In-order frame delivery with duplicate suppression on reconnect over a bounded, flow-controlled buffer; survives client roaming. **Cross-gateway HA** is real on a shared PostgreSQL store — identity-verified reattach failover, lease renewal + opt-in expiry-driven standby adoption (`session_failover: standby`), and a deliberate **live session move** (Transactional Handoff v2: prepare → ready → commit as one generation-CAS ownership swap). Idempotency is *enforced* server-side for framework-built backends (`mcp.Idempotency`), conveyed via `_meta` otherwise — meshmcp still does not claim exactly-once execution for foreign backends (see the delivery-vs-execution guarantees). |
...
| 🧾 **Gateway-signed audit** | Every decision is a hash-chained record sealed by **Ed25519-signed Merkle checkpoints**. `audit verify` reports one of four honest states — invalid, valid-but-untrusted-key, valid-but-unsealed-tail, or fully sealed. **Witnessed anchoring** (`audit verify --anchors`) escrows each sealed checkpoint to a file or an independent peer witness and exits non-zero on disagreement even when the chain seals internally — catching a key-holding insider's rewrite. Export to any OpenTelemetry/OTLP logs collector (`audit_otlp`) or scrape Prometheus metrics (`metrics_listen`); rotate the ledger by size (`audit_rotate_bytes`). |
...
| 🌐 **Scale & federate** | Aggregating router (LB · failover · discovery · bidirectional MCP · signed per-call **delegation** so an upstream verifies the original caller, not the router), a **multi-tenant** control plane (per-tenant policy/registry/audit partitioned by transport key), and identity-mapped cross-org federation with **SSO/OIDC** group mapping (`group:<sso-group>` policy rules driven by a verified OIDC token, additive over the WireGuard root). |
| 🔒 **HTTP at parity** | Streamable-HTTP backends enforce the same per-session **taint**, **secret injection + response/SSE redaction**, and **signed capabilities** as stdio; only DLP, shadow-policy, and delegation stay stdio-only. Backends can be jailed against out-of-band exfil via an operator `egress_wrapper` (`firejail --net=none`, bwrap, netns). |
...
| `control [flags]` | Managed control plane: node enrollment (NetBird key issuance), registry, policy distribution. **Multi-tenant** (`tenants:` ACL): per-tenant policy/registry/enrollment/audit partitioned by the transport key — see [docs/MULTI-TENANT.md](docs/MULTI-TENANT.md). |
| `federate --config <f>` · `federate usage --audit <f>` | Cross-org boundary: bridge granted tools between meshes, identity-mapped & audited; `usage` rolls up metered crossings per org/tool for billing export. |
...
| `fetch <peer:port> <sha256>` · `fetch --gc --dir <cas>` | Fetch a blob by content hash from a peer's content-addressed store; `--gc` garbage-collects the CAS by age/size (dry-run by default, `--apply` to delete). |
...
| `air <init·up·join·pair·database·home·nearby·announce·node·handoff·move·…>` | **Air Agent OS · Continuity**: scaffold and start a safe gateway, join/pair mesh identities, query through the governed database firewall, publish identity-stamped Presence + Activity cards, use Universal Actions to resolve verified nodes to live services, fan out to a `group:` of members (each individually governed), and move inert work context over exact-key-pinned device and agent hops. Resolved Send is receiver-confirmed; Handoff requires local acceptance and a destination-selected tool, keeps bounded delivery receipts, makes unknown delivery explicitly re-armable, and never pretends to transfer a live session. `air node` can host its own inbox/ring/screen/cast services. The same surface discovers, sends, steers, shares, rings, launches, approves, automates, and serves the responsive Air Home. Run `meshmcp air help` for every verb. |
| `air move grant\|list\|revoke <control:port> <dest-key> <session>` | **Transactional Handoff v2**: authorize a single-use *live* session-ownership move to a destination gateway. The move itself (prepare → ready → commit as one generation-CAS ownership swap, source serving until the swap) runs over the gateway control plane; the Control Room's drag-to-handoff fires it. See [docs/AIR-CONTINUITY.md](docs/AIR-CONTINUITY.md). |
...
| `audit verify <f> [--checkpoints --pubkey] [--anchors <f>]` | Verify a log: hash chain, or signatures + Merkle; `--anchors` cross-checks every sealed checkpoint against a witness file/peer, exiting non-zero on disagreement **even when the chain seals** — catching a key-holding insider's rewrite. |
...
| `capability issue --subject --audience --tool [--ttl] [--delegate-pub <k>]` | Sign a short-lived, subject-bound tool grant (present it with `call --capability @file`); `--delegate-pub` lets the holder mint attenuated single-use sub-grants (`capability delegate`) that can only narrow scope, never widen. |
...
| `config validate --config <f>` · `config lint --config <f>` · `doctor --config <f>` | Validate a config (globs/windows/enums/DLP); `lint` warns on allow-all rules, un-ratelimited egress tools, unapprovable cosign, glob secret grants, and shadowed rules; `doctor` runs pre-flight readiness checks (incl. a Postgres `session_store` reachability probe). |
...
| `room --audit <f> [--control <gateway>]` | Serve the interactive **Control Room** — live network (servers, agents, decision feed) **plus a console**: list/call tools over the mesh, a governed `run_command` terminal, and (opt-in `--local-shell`) a raw shell. With `--control`, live session tiles support **drag-to-handoff** — drag a session onto a destination to fire a Transactional Handoff v2 move (dual-authorized: room token + the room's own operator identity). |
| `connect` also ships as a thin `meshmcp-connect` binary | A minimal stdio ⇄ mesh bridge for MCP client configs — the connect path with none of the gateway/policy code compiled in (smaller footprint). |
...
- **[docs/spec/PAYMENT-EVIDENCE.md](docs/spec/PAYMENT-EVIDENCE.md)** — **x402 payment gating + payment evidence** *(Experimental)*: opt-in per-backend at the edge, a priced tool returns HTTP 402 with x402 `PaymentRequirements`, a verified `X-PAYMENT` forwards, and a `payment` receipt — **references only, never a wallet** — is written on the same signed audit record as the caller's mesh identity, correlating who-paid-for-which-call. A free dry-run route (`X-Meshmcp-Dry-Run`) lets a client prove compatibility before paying. Runs after the capability + policy double-gate; settlement is a pluggable facilitator.
