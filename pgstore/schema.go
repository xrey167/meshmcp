...
const (
	...
	// authorizes exactly one call fleet-wide. binding is a client+request
	// fingerprint of the redeeming call; result caches the served response so a
	// lost-response retry by the SAME binding is idempotent (a different binding
	// gets no result — it is a replay). Rows expire at expiry (a payment's
