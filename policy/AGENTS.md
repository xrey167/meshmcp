...
| `capability.go` | Signed Ed25519 capability grants: `CapabilityClaims`, `Signer.IssueCapability`, `CapabilityVerifier.Verify`, `stripCapability`, `applyCapability`. Fail-closed; pinned roots; subject/audience/tool-bound; single-use grants via `WithReplayGuard` (S19); only upgrades a policy-default deny. |
| `manifest.go` | Signed marketplace manifests (F14): `ManifestClaims`, `Signer.IssueManifest`, `ManifestVerifier.Verify`, `HashBundle`, `VerifyContent`. Mirrors the capability trust root (pinned authority keys, fail-closed) and binds a manifest to its bundle's content hash — governs plugin distribution, never runtime loading. |
