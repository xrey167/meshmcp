...
| `authorization`                 | OAuth 2.1 authorization layer: Protected Resource Metadata (RFC 9728), Authorization Server Metadata (RFC 8414/OIDC), Client ID Metadata Document, Dynamic Client Registration (RFC 7591), the token endpoint (`TokenRequest.Form()` for client_credentials / private_key_jwt / jwt-bearer, `TokenResponse`, `TokenErrorResponse`), plus the MCP discovery-URL ordering and a `WWW-Authenticate` challenge parser |
...
| `caching`                       | Draft result caching hints: `CacheableResult` (`ttlMs` / `cacheScope`) shared across all cacheable verbs (tools/list, resources/list, prompts/list, resources/read, server/discover) |
