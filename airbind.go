...
// bindMatch is the trigger: each field is a glob (path.Match) against the audit
// record's same field; an empty field matches anything. An all-empty match fires
// on every record (a universal reaction, e.g. a notifier).
...
// matchField reports whether a single glob pattern matches value. An empty
// pattern matches anything; an invalid pattern never matches (fail closed).
...
// fireBinding runs one matched rule's action. A print reaction writes a templated
// line; a run reaction spawns `meshmcp <expanded args...>` as a child (the same
// governed re-entry as `air launch`). run errors are reported, never fatal — one
// failing reaction must not stop the watch.
func fireBinding(b bindingRule, r streamRecord) {
	switch {
	case b.Do.Print != "":
		fmt.Println(dim(streamTime(r.Time)) + "  " + cyan("⇒ "+b.Name) + "  " + expandTemplate(b.Do.Print, r))
	case len(b.Do.Run) > 0:
		...
		fmt.Println(dim(streamTime(r.Time)) + "  " + amber("⇒ "+b.Name) + "  " + dim("run: meshmcp "+strings.Join(args, " ")))
...
func cmdAirBind(args []string) error {
	...
	auditPath := fs.String("audit", "", "audit JSONL to watch (default: the last positional arg's sibling; see usage)")
	...
	return followAudit(ctx, *auditPath, *fromStart, *interval, func(line []byte) {
